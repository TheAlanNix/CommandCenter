...
export default {
  ...
  methods: {
    getAmpData() {
      const path = `/api/amp/computer/${this.hostIp}`;
      axios.get(path)
        .then((res) => {
          console.log(res);
    ...
    setAmpGroup(groupGuid) {
      const path = `/api/amp/computer/${this.ampConnectorGuid}/group`;
      const data = {
    ...
    putAmpIsolation() {
      const path = `/api/amp/computer/${this.ampConnectorGuid}/isolation`;
      const data = {
